...
void BaseLevel::damage(){
    for(int i = 0 ; i < Rocks.size() ; i ++) {
        if (CheckMaskCollision(ship.get_image()     , ship.get_rect() ,
                               Rocks[i].get_image() , Rocks[i].get_rect()))
        {
            ship.Hp -= 5;
            HealthBarCol = RED;
            startCol = std::chrono::high_resolution_clock::now();
    ...
    for(int i = 0 ; i < Rocks.size() ; i ++){
        for (int j = 0 ; j < Rocks.size() ; j++){
            if(CheckMaskCollision(bullets[i].get_image() , bullets[i].get_rect() ,
                                  Rocks[j].get_image()   , Rocks[j].get_rect()))
            {
...
bool BaseLevel::CheckMaskCollision(Texture2D imgAt, Vector2 posA, Texture2D imgBt, Vector2 posB) {
